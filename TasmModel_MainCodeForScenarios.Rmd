...
### Fig. S6: model and observed biomases
...
#rm(list=ls())
library(ggplot2) # CN note need to run if starting from here 
library(dplyr) 
library(tidyverse) 
library(patchwork) 
...
# empircal values 
empir<-read.csv("plotData/ObservedBio_SizeChanges.csv")
#empir<-read.csv("plotData/ObservedBio_SizeChanges_CN.csv") # CN is the csv version
...
empirBio2<-empirBio1 %>% 
  mutate(group_trait ="benth_kappa")

empirBio3<-empirBio1 %>% 
  mutate(group_trait ="plankt_slope")

empirBio4<-empirBio1 %>% 
  mutate(group_trait ="benth_slope")
...
empirSize2<-empirSize1 %>% 
  mutate(group_trait = "benth_kappa")

empirSize3<-empirSize1 %>% 
  mutate(group_trait = "plankt_slope")

empirSize4<-empirSize1 %>% 
  mutate(group_trait = "benth_slope")

empirSize<-empirSize1 %>%
  full_join(empirSize2) %>% 
  full_join(empirSize3) %>% 
  full_join(empirSize4)
...
## define whether the plots are for main document (MS, kappa) or for supplementary information (SI, slope; including empirical values for changes in biomass and size)
...
#    summarise(mean = mean(value), sd = sd(value)) %>% 
    summarise(mean = mean(value), minQ = quantile(value, probs = 0.05, na.rm = T), maxQ = quantile(value, probs = 0.95, na.rm = T)) %>% 
    ungroup()
  ...
  # option 1 -order by ?
  spOrder<-rev(unique(b$plotName2)) ## CHANGE NAMES plotName1 
  b$plotName2<-factor(b$plotName2, levels = spOrder) ## CHANGE NAMES plotName1 

  # # option 2 - alphabetic order 
  # spOrder<-rev(sort(unique(b$plotName2))) ## CHANGE NAMES plotName1 
  # b$plotName2<-factor(b$plotName2, levels = spOrder) ## CHANGE NAMES plotName1 
  ...
  # if plots for main document ....
  if(plot_for_MS == "yes"){
    b<-b %>% filter(!group_trait %in% c("plankt_slope","benth_slope"),
                    !scenario == "empirical")
  }else if (plot_for_MS == "no"){
    b<-b %>% filter(!group_trait %in% c("plankt_kappa","benth_kappa"))
  }
  
  pd<-position_dodge(0.5)
  
  # check Caesioperca - outside plot limits for better visualisation. 
  # filter(b, plotName1== "Caesioperca", group_trait == "plankt_kappa", scenario %in% c("increase", "increase_warming"))
  
  ## CHANGE NAMES plotName1 
  plot<-ggplot(b, aes(x=mean, y=plotName2, group=scenario, color=scenario, shape = scenario))+
    geom_vline(xintercept = 0, linetype = "dashed")+
    geom_errorbar(aes(xmin=minQ, xmax=maxQ), width=0.5, position=pd)+
...
# adjust legend - if plot_for_MS == "no" you need to keep the biomass legend as yield has no emperical values.
if(plot_for_MS == "yes"){
...
# FIG 3 in MS
if(plot_for_MS == "yes"){
  name = "/Users/astaa/SMART projektas GTC Dropbox/Asta Audzijonyte (Gamtos Tyrimu Centras)/sizeSpectrum_projects/TasmanianProductivityChange/REVISION2/Fig3rev2.pdf"
}else if(plot_for_MS == "no"){
    name ="/Users/astaa/SMART projektas GTC Dropbox/Asta Audzijonyte (Gamtos Tyrimu Centras)/sizeSpectrum_projects/TasmanianProductivityChange/REVISION2/FigS7rev2.pdf"}

pdf(name, height=13, width=10)
