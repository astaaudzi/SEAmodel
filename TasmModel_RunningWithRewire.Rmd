...
This is the original code that was used to develop Tasmanian model and multiple resources for coastal ecosystems. This work was started in 2018 and later multiple resources and other features of this model (e.g. starvation mortality) were implemented in the main mizer package, through mizerMR and mizerStarvation add-ons. So it is NOT recommended to use this code, but it is kept here for tracing purposes. 
...
### Install mizer rewire, load libraries 

```{r warning=F, message=FALSE, echo=T}
#rm(list=ls())
#install.packages(vegan)
#remotes::install_github("astaaudzi/mizer", ref = "rewire-temp-comp")
library(mizerRewire)
library(tidyverse)
...
### Add senescence mortality function

Parameters used here are generally like in Law et al. 2009
...
You can explore model parameter development and calibration below, but first we look at baseline runs
...
mizerRewire::plot(tasm1)

#final biomasses
getBiomass(tasm1)[tmax,]

#model relative biomases compared to observed relative biomasess - we want them to be within 20% range
relbio <- (getBiomass(tasm1)[tmax,]/max(getBiomass(tasm1)[tmax,]))/mariaParams$scaledBio
relbio

# sizeat25cm <- mariaParams$a * 2.5^mariaParams$b
# 
#         MeanWgtAbove25 <- rep(NA,17)
#         for (xx in 1:length(mariaParams$species)) {
#             MeanWgtAbove25[xx]  <- mean(getMeanWeight(tasm1, min_w = sizeat25cm[xx], max_w = mariaParams$w_inf[xx])[(tmax-29):tmax])
#                   }

# get a value for observed invertebrate abundance based on empirical data (only for the size range used in the model)
...
#ratio of model vs observed inv abundances (benthic spectrum) in the modelled size range
modelInvAb/obsInvAb

```

### Run with fishing all species   

Next, we can run the same model with fishing. 
As a simplest step we fish ALL model species at once at the same level and use F = 0.2/year (but note that F in mizer is not directly comparable to F from stock assessments). We apply knife-edge selectivity at maturation size

```{r warning=FALSE, message=FALSE, echo=T}

# Fish all species at same level indicated in effort
tasm1ef <- project(params, t_max = tmax, effort=0.2, dt = 0.2, diet_steps = 0)  
plot(tasm1ef)
#since we are getting oscillations we get biomases averaged over the last 30 years
getBiomass(tasm1ef)[mean((tmax-30):tmax),]
#compare to unfished level
getBiomass(tasm1ef)[mean((tmax-30):tmax),]/getBiomass(tasm1)[tmax,]
```

### Variable F across species

Next we fish all model species using a vector of set mortalities. 
NOTE, we are changing params file here, so this will have to be reloaded if fixed mortality for all species is to be applied

```{r}
###
# now we fish species with different fishing mortality values, set in the vector
fish.mort.spp <- c(0.15, 0.15, 0, 0.1, 0.15, 0.1, 0.15, 0.1, 0.1, 0.15, 0.15, 0.15, 0.15, 0.15, 0.1, 0.15, 0.15)
# NOTE, we are changing params file here, so this will have to be reloaded if fixed mortality for all species is to be applied
diag(params@catchability) <- fish.mort.spp

tasm1ef <- project(params, t_max = tmax, effort=1, dt = 0.2, diet_steps = 0)  
#compare to unfished level
getBiomass(tasm1ef)[mean((tmax-30):tmax),]/getBiomass(tasm1)[tmax,]
...
This part is essential to assess species interactions. Now we fish one species at a time and see how that will affect the community? We fish one species in a loop and explore. We also save results in a dataframe 

While this is run in a loop you can check plots for each result. Note that species biomases are shown on a logarithmic scale, so small relative changes might be hard to see visually

!! NOTE - if you changed species cathabilities in the chunk above, rerun the chunk setting species params anew

```{r}
# what fishing mortality impose on each species individually? 
fish.mort <- 0.2
...
for (i in 1:length(params@species_params$species)) {
  ...
  print("Now fishing only")
  print(as.character(params@species_params$species[i]))
...
#print how relative biomasses changed from unfished level
print(round(getBiomass(tasm1ef1)[mean((tmax-30):tmax),]/getBiomass(tasm1)[tmax,],5))

#save them in a dataframe
temp <- round(getBiomass(tasm1ef1)[mean((tmax-30):tmax),]/getBiomass(tasm1)[tmax,],5)
