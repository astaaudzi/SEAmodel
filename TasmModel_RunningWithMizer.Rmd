...
prod_scen<-list( 
  # ignore scenarios without fishing
...
# no_predator: there are 16 species left
n_spp <- 16
...
fishonly <- as.character(mariaParams$species[c(1:14)])
...
for (scen in 1:scen_num) {
    ...
    for (iter in 1:dim(params28)[1]) {
        ...
        # no_predator: remove predator
        params <- removeSpecies(params, "predator")
        params@other_params$MR$interaction <-
            params@other_params$MR$interaction[c(1:14,16,17), ]
        ...
        PropLarFish <- rep(NA, n_spp)
        MeanWgtMat <- rep(NA, n_spp)
        MeanWgtAbove5 <- rep(NA, n_spp)
        MeanWgtAbove10 <- rep(NA, n_spp)
        for (xx in 1:n_spp) {
