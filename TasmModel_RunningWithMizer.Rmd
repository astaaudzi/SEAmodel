...
It is recommended to use standard mizer for all model runs, because this package is being maintained and developed, whereas mizerRewire was just a temporary project to get multiple background resources and temperature dependent rates. Now that multiple resources are available through mizerRewire experimental code on AstaAudzi GitHub account is not needed. 
...
tasm1ef <- project(params, t_max = tmax, effort=3, dt = dt) 
...
#emergent feeding level for a specific species
plotlyFeedingLevel(tasm1, include_critical = T, species = 6)
#emergent feeding level for all species
plotlyFeedingLevel(tasm1, include_critical = F)
...
## This is a cool animation of how spectra change through time. It is only interesting in the scenario with fishing because there we get oscillations. For the unfished scenario, where everything starts from equilibrium biomasses, spectra are stable. 
...
benthos_consumption <- array(data = 0, c(scen_num, dim(params28)[1]))
...
for (scen in 1:scen_num) {
    ...
    for (iter in 1:dim(params28)[1]) {
        ...
        # Calculate consumption rate of benthic resource
        # First calculate the per-capita rates in the final state
        pfinal <- params
        initialN(pfinal) <- finalN(tasm1)
        initialNResource(pfinal) <- finalNResource(tasm1)
        rfinal <- getRates(pfinal)
        # Then multiply the per capita resource mortality by the final
        # resource abundance and the mass and sum over all sizes to get
        # the total consumption rate
        benthos_consumption[scen, iter] <- 
          sum(rfinal$resource_mort["bb", ] * finalNResource(tasm1)["bb", ] *
                w_full(pfinal))
...
save(benthos_consumption, file = "outputs/benthos_consumption.RData")
...
Let's look at the change in consumption rates of benthic resource. We average
this over all parameter combinations.
```{r}
# Choose the scenarios chosen in the primer
scen_ben <- c(19, 25, 34, 24, 33, 28)

ratio = rep(0, 6)
for (i in 1:6) {
  ratio[i] <- mean(benthos_consumption[scen_ben[i], ] / 
                     benthos_consumption[scen_ben[1], ])
}
round(ratio, 2)
```
