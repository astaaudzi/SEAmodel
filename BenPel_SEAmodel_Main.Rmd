...
save(SppSurveyAll, file = "../inputs/SppSurveyLate.RData")
...
ModelSppStatisticsYear <- SppSurveyAll %>% group_by(TAXONOMIC_NAME, year) %>%
  summarise(meanBio = mean(bio16)/500, meanAbun = mean(abun16)/500)

#save(ModelSppStatisticsYear, file = "../outputs/annualBiomVariation.RData")
...
#Or match names for annual variation analyses
SppSurveyAll$modelName <- MatchNames$modelName[match(SppSurveyAll$TAXONOMIC_NAME, MatchNames$sppName)]


ModelSppStatisticsYear <- SppSurveyAll %>% group_by(modelName, year) %>%
  summarise(meanBio = mean(bio16)/500, meanAbun = mean(abun16)/500)

#save(ModelSppStatisticsYear, file = "../outputs/annualBiomVariationLate.RData")
...
#Annual Model species summary
ModelSpeciesAnnualVariation <- ModelSppStatisticsYear %>% group_by(modelName) %>% summarise(meanAb = mean(meanAbun), sdAb = mean(sdAbun), maxAb = mean(maxAbun), meanBio = mean(meanBio), sdBio = mean(sdBio), maxBio = mean(maxBio))
